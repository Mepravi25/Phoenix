...
# Extract signal coordinates from signal controllers
...
tree_actions = [] # (tree_obj, action, new_x, new_y)
...
for t in trees:
    ...
    if is_valid_tree_pos(tx, ty):
        ...
        tree_actions.append((t, 'keep', tx, ty))
    else:
        moved = False
        for r_m in (2.5, 4.0, 6.0, 8.0, 10.0, 12.0, 15.0, 18.0, 22.0, 26.0, 30.0):
            ...
            for a_i in range(num_angles):
                ...
                nx = round(tx + r_m * math.cos(ang), 2)
                ny = round(ty + r_m * math.sin(ang), 2)
                
                if is_valid_tree_pos(nx, ny):
                    add_tree_to_grid(nx, ny)
                    tree_actions.append((t, 'move', nx, ny))
                    moved = True
                    break
            if moved:
                break
        ...
        if not moved:
            tree_actions.append((t, 'remove', None, None))

# Apply changes to WBT file content in reverse order of start_pos so string indices remain valid
tree_actions.sort(key=lambda x: x[0]['start'], reverse=True)
...
for t, act, nx, ny in tree_actions:
    start_p, end_p = t['start'], t['end']
    old_str = t['str']
    
    if act == 'keep':
        continue
    elif act == 'move':
        # Replace translation tx ty tz in old_str
        new_str = re.sub(
            r'translation\s+[\d\.\-]+\s+[\d\.\-]+\s+([\d\.\-]+)',
            f'translation {nx:.2f} {ny:.2f} \\1',
            old_str,
            count=1
        )
        modified_text = modified_text[:start_p] + new_str + modified_text[end_p:]
        moves_done += 1
    elif act == 'remove':
        # Remove tree solid block and any trailing whitespace/newlines
        # Check if preceded or followed by newlines
        modified_text = modified_text[:start_p] + modified_text[end_p:]
        removes_done += 1

print(f"Applied updates to file text: {moves_done} moved, {removes_done} removed.")

# Save modified WBT file
with open(wbt_path, "w", encoding="utf-8") as f:
    f.write(modified_text)

print(f"Successfully saved updated {wbt_path}!")
